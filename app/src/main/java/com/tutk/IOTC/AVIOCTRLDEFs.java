...
public class AVIOCTRLDEFs {
	...
	public static class SFrameInfo {
		...
		public static SFrameInfo parseContent(byte[] p) {
			if (p == null || p.length <16)
				return null;
			SFrameInfo c = new SFrameInfo();
			c.codec_id = Packet.byteArrayToShort_Little(p, 0);
			c.flags = p[2];
			c.cam_index = p[3];
			c.onlineNum = p[4];
			c.timestamp = (0xff & p[12]) | (0xff & p[13]) <<8 |
					(0xff & p[14]) << 16 | (0xff & p[15]) << 24;

			return c;
		}
		public static byte[] packContent(short codec_id, byte flags, byte cam_index, byte online_num, int timestamp) {
